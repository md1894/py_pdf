...
for row in tran_group:
    ...
    pdf.cell(20, 2*th, str(tran_group[cnt][1]), border=1, fill = 1)
...
pdf.output(fname, 'F')
add_encryption(input_pdf=fname, output_pdf=fname[1:], password=account_no[-6:])
os.system('rm -rf %s'%(fname))
os.system('evince %s &'%(fname[1:]))